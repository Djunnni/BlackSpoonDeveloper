  ...
  return (
    <Layout>
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* 상단 헤더 (뒤로가기 + 중앙 타이틀) */}
        <div className="sticky top-0 z-50 bg-gray-50/80 backdrop-blur pb-4">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              ...
              <button
                type="button"
                onClick={() => setInterestAlert((v) => !v)}
                className={`relative w-12 h-6 rounded-full transition-colors ${
                  interestAlert ? "bg-blue-600" : "bg-gray-300"
                }`}
                aria-label="이자 발생 알림 토글"
              >
                <div
                  className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
            ...
            <div className="flex items-center justify-between p-4">
              ...
              <button
                type="button"
                onClick={() => setZoneChangeAlert((v) => !v)}
                className={`relative w-12 h-6 rounded-full transition-colors ${
                  zoneChangeAlert ? "bg-blue-600" : "bg-gray-300"
                }`}
                aria-label="존 변경 알림 토글"
              >
                <div
                  className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
